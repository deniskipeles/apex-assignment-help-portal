...
export default function Home() {
  ...
  return (
    <div className="bg-slate-50/50" id="landing_page">
      {/* Hero Section */}
      <section className="relative overflow-hidden py-16 sm:py-24 bg-white border-b border-slate-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-3xl mx-auto space-y-6">
            <div className="inline-flex items-center gap-1.5 bg-sky-50 border border-sky-100/80 px-3.5 py-1.5 rounded-full text-sky-700 text-xs font-semibold uppercase tracking-wider">
              <Sparkles className="w-3.5 h-3.5" /> Trusted by 10,000+ university students
            </div>
            
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-900 heading-font leading-tight">
              Get Expert Homework Help <br />
              With <span className="text-sky-600">Secure Escrow Payments</span>
            </h1>
            ...
            <div className="pt-4 flex flex-col sm:flex-row gap-3 justify-center items-center">
              {isAuthenticated ? (
                ...
                <>
                  ...
                  <Link
                    to="/auth"
                    className="w-full sm:w-auto bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 font-bold px-7 py-3.5 rounded-2xl transition-colors text-center"
                  >
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-2 mb-12">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 heading-font tracking-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {features.map((feat, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white border border-slate-200/60 p-6 sm:p-8 rounded-2xl shadow-sm hover:shadow-md transition-shadow flex flex-col items-start gap-4"
              >
                <div className={`p-3 rounded-2xl ${feat.color}`}>
                  <Icon className="w-6 h-6" />
                </div>
                <h4 className="font-bold text-slate-800 text-base heading-font">{feat.title}</h4>
      ...
      <section className="py-16 bg-white border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6">
            <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 font-semibold text-xs uppercase tracking-wider">
              <ShieldCheck className="w-3.5 h-3.5" /> 100% Escrow Guaranteed
            </div>
            
            <h2 className="text-2xl sm:text-4xl font-extrabold text-slate-900 heading-font tracking-tight leading-tight">
              No Risk. Pay Only For <span className="text-emerald-600">Approved Solutions</span>
            </h2>
            ...
            <div className="pt-3">
              <Link
                to="/auth?register=true"
                className="inline-flex items-center gap-1 bg-slate-950 text-white font-bold text-xs px-5 py-3 rounded-xl hover:bg-slate-800 shadow-md cursor-pointer transition-colors"
              >
          ...
          <div className="bg-slate-50 p-6 sm:p-8 rounded-3xl border border-slate-200/60 grid grid-cols-2 gap-4">
            <div className="bg-white p-5 rounded-2xl border border-slate-200/40 shadow-sm space-y-2">
              <Flame className="w-5 h-5 text-amber-500" />
              <span className="text-xl font-bold text-slate-800 heading-font block">45 min</span>
              <span className="text-[10px] text-slate-400 font-bold uppercase block tracking-wider">Average Tutor Bid Time</span>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-slate-200/40 shadow-sm space-y-2">
              <Users className="w-5 h-5 text-sky-500" />
              <span className="text-xl font-bold text-slate-800 heading-font block">150+</span>
              <span className="text-[10px] text-slate-400 font-bold uppercase block tracking-wider">Active PhD Tutors</span>
            </div>
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {tutors.slice(0, 4).map((tutor) => (
            <div
              key={tutor.id}
              className="bg-white border border-slate-200/80 p-5 rounded-2xl text-center flex flex-col items-center gap-3.5 shadow-sm"
            >
              <img
                src={tutor.avatar}
                alt={tutor.name}
              ...
              />
              <div>
                <h4 className="font-bold text-slate-800 text-sm heading-font leading-none">{tutor.name}</h4>
                <p className="text-[10px] bg-sky-100 text-sky-700 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider w-max mx-auto mt-2">
                  ${tutor.hourlyRate}/hr
                </p>
              </div>
              ...
              <div className="flex items-center gap-1.5 justify-center">
                ...
                <span className="font-bold text-xs text-slate-700">{tutor.rating}</span>
                <span className="text-slate-300">•</span>
                <span className="text-xs text-slate-400 font-medium">{tutor.completedTasks} tasks</span>
              </div>
              ...
              <div className="flex flex-wrap gap-1 justify-center pt-1.5">
                {tutor.expertise?.slice(0, 3).map((exp, i) => (
                  <span
                    key={i}
                    className="bg-slate-50 border border-slate-200 text-slate-600 text-[9px] font-bold px-2 py-0.5 rounded-md"
                  >
                    {exp}
                  </span>
                ))}

...
export default function MessageWindow() {
  ...
  if (!activeChatRoomId || !activeChatPartner) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-slate-50 border border-slate-200/50 rounded-3xl" id="empty_chat_view">
        <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mb-4 text-slate-400">
          <Send className="w-8 h-8" />
        </div>
        <h4 className="text-base font-bold text-slate-800 heading-font mb-1">Select a Tutoring Conversation</h4>
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm" id="active_chat_window">
      {/* Chat Room Header */}
      <div className="px-5 py-4 bg-slate-50 border-b border-slate-200/80 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <h4 className="font-bold text-slate-800 text-sm leading-none heading-font">{activeChatPartner.name}</h4>
        ...
        {activeChatPartner.role === 'tutor' && (
          <div className="text-right">
            <span className="text-xs text-slate-400 block leading-none">Hourly rate</span>
            <span className="text-sm font-bold text-sky-600 leading-normal block">
      ...
      {/* Safety Banner */}
      <div className="bg-sky-50 px-4 py-2 text-[11px] text-sky-800 font-medium flex items-center gap-1.5 border-b border-sky-100">
          ...
          messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[9px] text-slate-400 mt-1 mx-2">
      ...
      {attachedFile && (
        <div className="px-5 py-2.5 bg-sky-50/80 border-t border-sky-100 flex items-center justify-between text-xs font-semibold text-slate-700">
      ...
      {/* Chat inputs footer */}
      <form onSubmit={handleSend} className="px-5 py-3.5 border-t border-slate-200 flex items-center gap-3">
        {/* Hidden inputs */}
        <input
          type="file"
          ref={fileInputRef}
        ...
        <button
          ...
          className="text-slate-400 hover:text-sky-600 p-2 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer shrink-0"
        ...
        <input
          ...
          className="flex-1 bg-slate-50 border border-slate-200 focus:border-sky-500 focus:bg-white text-sm text-slate-800 px-4 py-2.5 rounded-2xl outline-none transition-all"
        />
        ...
        <button
          ...
          className="bg-sky-600 text-white p-2.5 rounded-2xl hover:bg-sky-700 disabled:bg-slate-200 disabled:text-slate-400 shadow-md shadow-sky-500/10 cursor-pointer transition-colors shrink-0"
        >

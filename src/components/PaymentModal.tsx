...
export default function PaymentModal({ isOpen, onClose }: PaymentModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="relative w-full max-w-md overflow-hidden bg-white rounded-3xl shadow-2xl border border-slate-100 flex flex-col max-h-[90vh]">
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center justify-center text-center p-8 min-h-[300px]">
            <div className="w-16 h-16 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mb-4 border border-emerald-100 animate-bounce">
              <CheckCircle2 className="w-10 h-10" />
            </div>
            <h4 className="text-xl font-bold text-slate-800 heading-font mb-2">Deposit Successful!</h4>
            <p className="text-sm text-slate-500 max-w-xs mb-1">
              Your secure transfer of <span className="font-semibold text-slate-800">${amount}</span> has been processed.
            </p>
            <span className="text-[10px] font-mono text-slate-400">ID: TX_SECURE_{Math.random().toString(36).substr(2, 9).toUpperCase()}</span>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-sky-500 focus:bg-white text-slate-800 font-semibold pl-8 pr-4 py-2.5 rounded-2xl outline-none transition-all text-sm"
            ...
            <div className="space-y-3.5 pt-2 border-t border-slate-100">
              ...
              <div>
                <input
                  ...
                  className="w-full border border-slate-200 focus:border-sky-500 px-3.5 py-2.5 rounded-xl outline-none text-sm text-slate-800"
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full border border-slate-200 focus:border-sky-500 pl-10 pr-4 py-2.5 rounded-xl outline-none text-sm text-slate-800"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <input
                    ...
                    className="w-full border border-slate-200 focus:border-sky-500 px-3.5 py-2.5 rounded-xl outline-none text-sm text-slate-800 text-center"
                    placeholder="MM/YY"
                    required
                  />
                </div>
            ...
            <div className="bg-slate-50 border border-slate-200/50 p-3.5 rounded-2xl flex gap-2.5 text-xs text-slate-500">
              <ShieldCheck className="w-5 h-5 text-emerald-600 shrink-0" />
              <div>
                <span className="font-semibold text-slate-700 block">EduSolve Protection Guarantee</span>
                Your assignment funds are locked safely in escrow and are only released to the tutor once you formally approve the completed solution files.
              </div>
            </div>
